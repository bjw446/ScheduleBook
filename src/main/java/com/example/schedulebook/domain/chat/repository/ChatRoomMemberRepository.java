...
public interface ChatRoomMemberRepository extends JpaRepository<ChatRoomMember, Long> {
    ...
    @Query("SELECT crm.user.id as userId, crm.user.nickname as nickname, " +
            "crm.lastReadMessageId as lastReadMessageId FROM ChatRoomMember crm " +
            "WHERE crm.chatRoom.id = :roomId AND crm.user.id <> :currentUserId AND crm.deletedAt IS NULL")
