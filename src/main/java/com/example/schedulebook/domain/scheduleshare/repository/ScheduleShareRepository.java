...
public interface ScheduleShareRepository extends JpaRepository <ScheduleShare, Long> {
    ...
    @Query("SELECT ss FROM ScheduleShare ss JOIN FETCH ss.schedule s JOIN FETCH s.user WHERE ss.sharedUser.id = :userId AND ss.scheduleShareStatus = :status ")
    ...
    @Query("SELECT ss FROM ScheduleShare ss JOIN FETCH ss.schedule s JOIN FETCH s.user WHERE ss.id = :shareId AND ss.scheduleShareStatus = :status ")
    Optional<ScheduleShare> findActiveShareDetail(@Param("shareId") Long shareId, @Param("status") ScheduleShareStatus status);

    @Query("SELECT ss FROM ScheduleShare ss JOIN FETCH ss.schedule s JOIN FETCH ss.sharedUser WHERE s.user.id = :ownerId AND ss.scheduleShareStatus = :status ")
    List<ScheduleShare> findOwnedShares(@Param("ownerId") Long ownerId, @Param("status") ScheduleShareStatus status);

    @Query("SELECT ss FROM ScheduleShare ss JOIN FETCH ss.schedule s JOIN FETCH s.user JOIN FETCH ss.sharedUser WHERE ss.id = :shareId AND ss.scheduleShareStatus = :status ")
