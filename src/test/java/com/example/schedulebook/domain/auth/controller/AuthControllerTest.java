...
class AuthControllerTest {
    ...
    void setUp() throws Exception {
        ...
        doAnswer(invocation -> {
            ...
            chain.doFilter(invocation.getArgument(0), invocation.getArgument(1));
            return null;
        }).when(cachedBodyFilter).doFilter(any(ServletRequest.class), any(ServletResponse.class), any(FilterChain.class));
    }
    ...
    void 회원가입_성공() throws Exception {
        SignupRequest request = new SignupRequest("test123", "Password1!", "테스트유저", "test@example.com", "010-1234-5678");
        SignupResponse response = new SignupResponse("test123", "테스트유저", "test@example.com", "010-1234-5678");
    ...
    void 회원가입_유효하지_않은_요청이면_400() throws Exception {
        SignupRequest request = new SignupRequest("", "short", "", "invalid-email", "invalid");
    ...
    void 로그인_성공() throws Exception {
        LoginRequest request = new LoginRequest("test123", "Password1!", null);
        LoginResponse response = new LoginResponse(1L, "테스트유저", 1, "access-token", "refresh-token");
    ...
    void 토큰_재발급_성공() throws Exception {
        ...
        LoginResponse response = new LoginResponse(1L, "테스트유저", 1, "new-access-token", "new-refresh-token");

        given(authService.refresh(any(RefreshRequest.class), any())).willReturn(response);
    ...
    void 내_세션_조회_성공() throws Exception {
        Long userId = 1L;
        setAuthenticatedUser(userId); // 헬퍼 메서드 사용으로 중복 제거

        List<SessionInfoResponse> responses = List.of(
                new SessionInfoResponse("session-1", "127.0.0.1", "Chrome", null, null)
        );

        given(authService.findMySessions(userId)).willReturn(responses);
    ...
    void 특정_세션_로그아웃_성공() throws Exception {
        ...
        willDoNothing().given(authService).logoutSession(eq(userId), eq(sessionId), any());
    ...
    void 특정_세션_로그아웃_인증정보가_없으면_401() throws Exception {
        mockMvc.perform(delete("/auth/sessions/{sessionId}", "session-1")
                        .with(csrf()))
                .andExpect(status().isUnauthorized());
    }
}